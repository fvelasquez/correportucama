...
j(function(){
	...
	*/
	j('#appdowncounternums').countdown({until: new Date(2011, 8 - 1, <?php echo '12';//echo date('d') ?>, 15, 00, 00),compact: true,format: 'HMS'});
...
<?php
	...
	if($pista == '7'){ $limit = "LIMIT 200,33"; }
	if($pista == '8'){ $limit = "LIMIT 233,33"; }
	if($pista == '9'){ $limit = "LIMIT 266,34"; }
	if($pista == '10'){ $limit = "LIMIT 300,33"; }
	if($pista == '11'){ $limit = "LIMIT 333,33"; }
	if($pista == '12'){ $limit = "LIMIT 366,34"; }
	...
	while($r = mysql_fetch_assoc($res2)){
		...
		echo "
		#corredor_".$i."{
			background: url(images/cuerpo_".$j.".png) no-repeat top center;
			height: 50px;
			width: 50px;
			float: left;
			position: relative;
...
<div id="appbody">
	...
	<?php 
		...
		if($totpart > 233 && $pista == '7'){ $appnextpista = 1; }
		if($totpart > 266 && $pista == '8'){ $appnextpista = 1; }
		if($totpart > 300 && $pista == '9'){ $appnextpista = 1; }
		if($totpart > 333 && $pista == '10'){ $appnextpista = 1; }
		if($totpart > 366 && $pista == '11'){ $appnextpista = 1; }
		if($totpart > 400 && $pista == '12'){ $appnextpista = 1; }
	...
	<?php
		...
		if($pista == '6') { $pistanexticon = 'pista_next_7.png'; $pistalink = '7'; }
		
		if($pista == '7') { $pistanexticon = 'pista_next_8.png'; $pistalink = '8'; }
		if($pista == '8') { $pistanexticon = 'pista_next_9.png'; $pistalink = '9'; }
		if($pista == '9') { $pistanexticon = 'pista_next_10.png'; $pistalink = '10'; }
		if($pista == '10') { $pistanexticon = 'pista_next_11.png'; $pistalink = '11'; }
		if($pista == '11') { $pistanexticon = 'pista_next_12.png'; $pistalink = '12'; }
		if($pista == '12') { $pistanexticon = 'pista_next_1.png'; $pistalink = '1'; }
	?>
